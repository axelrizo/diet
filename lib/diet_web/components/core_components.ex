defmodule DietWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: DietWeb.Gettext
  ...
  def error(assigns) do
    ~H"""
    <p data-test-id="error" class="flex gap-3 mt-3 text-sm leading-6 text-rose-600">
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 data-testid="header" class="text-lg font-semibold leading-8 text-zinc-800">
          {render_slot(@inner_block)}
        </h1>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DietWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DietWeb.Gettext, "errors", msg, opts)
    end
  end
